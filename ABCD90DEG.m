...
elseif strcmpi(in, 'C')
    ...
    PartC(warpedImg, start_pos, goal_pos, obstacles, robot, tilt_angle,vacuum);
...
function PartC(warpedImg, start_pos, goal_pos, obstacles, robot, tilt_angle)  % FIXED: Parameter name
    % ---- Robot constants ----
...
function PartD(warpedImg, start_pos, goal_pos, obstacles, robot, tilt_angle, vacuum)

    % ---- Robot constants ----
    home = [-588.53, -133.30, 227.00, 2.221, 2.221, 0];

    lift_height         = 12;    % REQUIRED: transport & place height (mm)
    approach_height     = 3;   % vertical hop (mm)
    intermediate_height = 50;   % hover just to rotate safely (mm)

    fprintf('\n=== PART D: Pick and Place with 90° Rotation ===\n');
    fprintf('Start position (img): [%.2f, %.2f]\n', start_pos);
    fprintf('Goal position  (img): [%.2f, %.2f]\n', goal_pos);
    fprintf('Original tilt angle: %.2f deg\n', tilt_angle);

    % ---- Move to home ----
    robot.movej(home);

    % ---- Plan path with APF (image frame) ----
    pathXY = PartB(warpedImg, start_pos, goal_pos, obstacles);
    ...
    % de-dup close points to avoid controller corner-cutting
    if ~isempty(robot_path)
        keep = [true; sqrt(sum(diff(robot_path).^2,2)) > 3]; % >3 mm apart
        robot_path = robot_path(keep,:);
    end

    fprintf('Robot start (mm): [%.2f, %.2f]\n', robot_start);
    fprintf('Robot goal  (mm): [%.2f, %.2f]\n', robot_goal);

    % ---- HARD arrive at start so it never "skips" ----
    robot.movej([robot_start(1), robot_start(2), approach_height, home(4:6)]);
    robot.movej([robot_start(1), robot_start(2), lift_height,     home(4:6)]);

    % ---- Build pick sequence (start) ----
    path = [];
    path = [path; robot_start(1), robot_start(2), approach_height, home(4:6)]; % above
    path = [path; robot_start(1), robot_start(2), lift_height,     home(4:6)]; % pick height

    % (grip here on real robot)
     vacuum.grip(); pause(0.3);

    % hover up to rotate safely
    path = [path; robot_start(1), robot_start(2), intermediate_height, home(4:6)];

    % ---- Execute pick sequence (edge waypoints: no blend) ----
    rtde_path = [];
    v = 0.5; a = 1.2;
    for i = 1:size(path,1)
        b = 0; if i>1 && i<size(path,1), b = 0.002; end  % blend only in middle
        rtde_path = [rtde_path; [path(i,:), a, v, 0, b]];
    end
    poses = robot.movej(rtde_path);
    robot.drawPath(poses);

    % ---- Rotate by 90° for obstacle clearance (subtract original tilt) ----
    current_joints    = robot.actualJointPositions();
    current_joints_deg= rad2deg(current_joints);
    rotation_90_deg   = 90;
    new_joints_deg    = current_joints_deg;
    new_joints_deg(6) = new_joints_deg(6) + rotation_90_deg - tilt_angle;
    robot.movej(deg2rad(new_joints_deg), 'joint'); pause(0.2);

    % Freeze this rotated orientation for transport
    rotated_pose     = robot.actualPosePositions();  % [x y z rx ry rz]
    transport_orient = rotated_pose(4:6);

    % ---- Transport along APF path at 6 mm with fixed orientation ----
    path_transport = [];
    for i = 1:size(robot_path,1)
        path_transport = [path_transport; robot_path(i,1), robot_path(i,2), lift_height, transport_orient];
    end
    % final transport point over goal (still 6 mm)
    path_transport = [path_transport; robot_goal(1), robot_goal(2), lift_height, transport_orient];

    rtde_path_transport = [];
    for i = 1:size(path_transport,1)
        b = 0; if i>1 && i<size(path_transport,1), b = 0.002; end
        rtde_path_transport = [rtde_path_transport; [path_transport(i,:), a, v, 0, b]];
    end
    poses = robot.movej(rtde_path_transport);
    robot.drawPath(poses);

    % ---- Final orientation: undo 90° and apply goal tilt ----
    q     = robot.actualJointPositions();
    q_deg = rad2deg(q);
    q_deg(6) = q_deg(6) - rotation_90_deg + tilt_angle;
    robot.movej(deg2rad(q_deg), 'joint'); pause(0.2);

    % Read the turned orientation we just set
    turned_pose   = robot.actualPosePositions();
    turned_orient = turned_pose(4:6);

    % ---- Place at 6 mm with turned orientation ----
    place_path = [robot_goal(1), robot_goal(2), lift_height, turned_orient];
    robot.movej(place_path);

    % (release here on real robot)
     vacuum.release(); pause(0.2);

    % ---- Lift away using turned orientation, not home ----
    final_lift = [robot_goal(1), robot_goal(2), approach_height, turned_orient];
    robot.movej(final_lift);

    % ---- Return home ----
    robot.movej(home);

    % quick checks (optional for console)
    fprintf('Transport Z unique (expect 6): '); disp(unique(path_transport(:,3))');
    fprintf('First 4 transport pts:\n'); disp(path_transport(1:min(4,end),:));
end
